...
async function exportToPDF() {
  ...
  try {
    console.log('[RelatoriosManager] ===== INICIANDO EXPORTAÇÃO PDF =====')
    console.log('[RelatoriosManager] relatoriosData.value:', relatoriosData.value?.length || 0, 'registros')
    console.log('[RelatoriosManager] relatoriosFiltrados.value:', relatoriosFiltrados.value?.length || 0, 'registros')
    console.log('[RelatoriosManager] Dados completos relatoriosData:', relatoriosData.value)
    console.log('[RelatoriosManager] Dados filtrados:', relatoriosFiltrados.value)
    
    // Usar o composable useRelatorioPDF
    console.log('[RelatoriosManager] Iniciando exportação PDF com', relatoriosFiltrados.value.length, 'registros')
    
    await gerarPDF(relatoriosFiltrados.value, {
      dataInicio: filtros.value.dataInicial,
      dataFim: filtros.value.dataFinal,
      agente: filtros.value.agenteOuContato,
      classificacao: filtros.value.classificacao
    })
    
    console.log('[RelatoriosManager] PDF gerado com sucesso!')
  } catch (error) {
    console.error('[RelatoriosManager] Erro ao exportar PDF:', error)
